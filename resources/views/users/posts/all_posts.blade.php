...
<section class="d-flex justify-content-center">
        ...
        <div class="card bg-light mb-3" style="max-width: 35rem;">
            ...
            <div class="card-header card-top">
                ...
                @if (isset($fixed_comments_post->file))
...
@foreach ($comments_posts as $comments_post)
    ...
    <section class="d-flex justify-content-center">
            ...
            <div class="card bg-light mb-3" style="max-width: 35rem;">
                ...
                <div class="card-header card-top">
                    <img src="{{asset('images/profile/'.$comments_post->admins->photo)}}" alt="loading" class="rounded-circle">
                ...
                <a class="comment_link"   post_id={{$comments_post->id}}>
                    <div class="card-body d-flex justify-content-center comment_btn" id="{{$comments_post->id}}" style="background-color:#eee ">
                        <i class="fas fa-comment"></i>Comment
                    </div>
                </a>
                ...
                <div class="card-header card-bottom">
                    @if ($comments_post->comments_count != 0)
                        <small class=" {{'view_comments '.$comments_post->id}}" id="{{'view'.$comments_post->id}}" >view all comments</small>
                    @endif
                    
                    @foreach ($comments_post->comments as $comment)
                        <div class="{{'comment com'.$comments_post->id}}" id="{{'comm'.$comment->id}}" style="display: none">
                            @if (isset($comment->users->photo))
                                <img src="{{asset('images/profile/'.$comment->users->photo)}}" alt="loading" class="rounded-circle">
                                <span >{{$comment->users->name}}</span>
                            @else
                    ...
                    <form id="{{'form_comment'.$comments_post->id}}" >
                        @csrf
                        <img src="{{asset('images/profile/'.Auth::user()->photo)}}" alt="loading" class="rounded-circle img_input">
                        <textarea name="comment" post_id="{{$comments_post->id}}" id="{{'input'.$comments_post->id}}" cols="20" rows="2" class="form-control input"></textarea>
